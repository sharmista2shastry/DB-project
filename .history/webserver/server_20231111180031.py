...
def getCardholderDetails():
    email = request.json['email']
    params_dict = {"email":email}
    cursor = g.conn.execute(text("SELECT CARDHOLDER_NAME FROM CARDHOLDER_DETAILS WHERE EMAIL=(:email);"), params_dict)
    g.conn.commit()
    ...
    cursor = g.conn.execute(text("SELECT CARD_NUMBER, AVAILABLE_FUNDS FROM CARDHOLDER_DETAILS WHERE EMAIL=(:email);"), params_dict)
    g.conn.commit()
