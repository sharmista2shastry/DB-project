...
def paywithtoken():
    ...
    isValid = False
    try:
      cursor = g.conn.execute(text("SELECT S.CARD_TOKEN FROM internetflix_stored_card_data S WHERE S.STORED_CARD_ID = (SELECT A.STORED_CARD_ID FROM INTERNETFLIX_CUSTOMER_DATA A WHERE A.EMAIL=(:email)) AND S.MERCHANT_ID=(SELECT MERCHANT_ID FROM MERCHANTS);"), params_dict)
