...
def paywithcard():
    ...
    params_dict = {"email":email, "amount": amount, "cvv": cvv, "expiry":expiry, "card_number":cardNumber}
    isValid = False
    try:
      cursor = g.conn.execute(text("SELECT CHECK_CARD_MATCH(:email, :card_number, :expiry, :cvv, SELECT(M.MERCHANT_ID FROM MERCHANTS M WHERE M.COUNTRY_ID = SELECT C.COUNTRY_ID FROM COUNTRIES C WHERE COUNTRY=SELECT CHECK_CARD_MATCH('vjorry0@yahoo.co.jp', '1234-5696-3702-9692', '2024-07-02', 789, 1);
));"), params_dict)
