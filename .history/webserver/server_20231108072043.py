...
def paywithcard():
    ...
    try:
      ...
      if isValidCard:
        ...
        cursor = g.conn.execute(text("SELECT PROCESS_TRANSACTION(:email, :card_number, :amount, SELECT M.MERCHANT_ID INTO merchant_id FROM MERCHANTS M WHERE M.MERCHANT_NAME ILIKE ('%Internetflix Ltd.%') AND M.COUNTRY_ID = (SELECT C.COUNTRY_ID FROM COUNTRIES C WHERE C.COUNTRY=$5), :transaction_id);"), params_dict)
