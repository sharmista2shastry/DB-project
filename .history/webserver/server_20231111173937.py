...
def getcards():
    email = request.json['email']
    params_dict = {"email":email}
    cursor = g.conn.execute(text("SELECT CUSTOMER_NAME FROM CARDS C JOIN CARDHOLDER_DETAILS CD ON C.CARDHOLDER_ID = CD.CARDHOLDER_ID WHERE CD.EMAIL=(:email);"), params_dict)
    g.conn.commit()
