...
def paywithcard():
    ...
    isValid = False
    try:
      cursor = g.conn.execute(text("SELECT CHECK_CARD_MATCH(:email, :card_number, :expiry, :cvv, SELECT(M.MERCHANT_ID FROM MERCHANTS M WHERE M.COUNTRY_ID = SELECT C));"), params_dict)
