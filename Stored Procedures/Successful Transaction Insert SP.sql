CREATE OR REPLACE FUNCTION insert_successful_transactions_for_cards()
RETURNS void AS
$$
DECLARE
    card_row RECORD;
...
BEGIN
    ...
    FOR card_row IN (SELECT * FROM CARDS) LOOP
		random_num := floor(random() * 11);
		-- Add 10 to the random number to get a range between 10 and 20
		random_num := random_num + 10;
		WHILE loop_counter < random_num LOOP
			rand_multiplier := floor(random() * 300);
			-- Insert a row in the TRANSACTIONS table for each card
			INSERT INTO TRANSACTIONS (AMOUNT, CURRENCY, FRAUD, TRANSACTION_STATUS, TRANSACTION_TIMESTAMP, MERCHANT_ID, ACQUIRER_ID, TRANSACTION_TYPE_ID, CARD_NUMBER, CARDHOLDER_ID, AUTHENTICATION_TYPE_ID)
			VALUES (
				...
				TRUE,   -- Example: Transaction status
...
SELECT insert_successful_transactions_for_cards();
